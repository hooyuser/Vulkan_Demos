...
private:
	...
	void saveCubemap() {
		VkResult res = vkWaitForFences(device, 1, &renderFence, VK_TRUE, 600000000000);
		...
		size_t imageBytes = texWidth * texHeight * 4 * 1;  //4 channel, sizeof(srbg) == 1Byte 

		const char* imagedata;
		float* outputImage = (float*)malloc(imageBytes);

		if (subResourceLayout.rowPitch == texWidth * 4 * 1) {
			vkMapMemory(device, stagingImageMemory, 0, imageBytes, 0, (void**)&imagedata);
			imagedata += subResourceLayout.offset;
			memcpy(outputImage, imagedata, imageBytes);
		}
		else {
			vkMapMemory(device, stagingImageMemory, 0, subResourceLayout.size, 0, (void**)&imagedata);
			imagedata += subResourceLayout.offset;
			char* pStart = (char*)imagedata;
			for (int i = 0; i < texHeight; i++) {
				memcpy(outputImage, pStart, texWidth * 4 * 1);
				pStart += subResourceLayout.rowPitch;
			}
		}
		vkUnmapMemory(device, stagingImageMemory);

		file_io::saveImage(filename, outputImage, texWidth, texHeight, output_format);
	...
	void createOutputImages() {
		...
		createImage(texWidth, texHeight, stagingImageFormat, VK_IMAGE_TILING_LINEAR, VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingImage, stagingImageMemory);
	}
	...
	void createAttachmentImageViews() {
		colorAttachmentImageView = createImageView(colorAttachmentImage, colorAttachmentFormat, VK_IMAGE_ASPECT_COLOR_BIT);
	}
